    ...
    try:
        ...
        if submission.over_18 is True:
            ...
            print(f"""
                ...
                URL:{submission.url}""")
            Embed = discord.Embed(title=f"**{submission.title}** From:**{submission.author}** in: **{sub}**",description=f"Current score of {submission.score}. Look at the post here {submission.url}", color= discord.Color.red()).set_image(url=submission.url)
       ...
       await ctx.send("You must specify a **REAL** subreddit(Ex: ``Linda browse_reddit ProgrammerHumor`` **OR** ``Linda reddit ProgrammerHumor``)")
